...
import {
  ...
  SANDBOX_STRIPE_AUTH_WEBHOOK_SECRET,
...
async function getUserEmailByReferenceId(
  referenceId: string
): Promise<string | null> {
  try {
    const user = await db
...
const stripeClient = new Stripe(
  ENVIRONMENT === "production" ? STRIPE_SECRET_KEY : SANDBOX_STRIPE_SECRET_KEY,
  {
    ...
    after: createAuthMiddleware(async (ctx) => {
      ...
      if (ctx.path.startsWith("/callback") && ctx.context.newSession?.user) {
        ...
        const [userData, existingLimits] = await Promise.all([
          ...
          db.query.monthlyLimits.findFirst({
        ...
        const needsIpUpdate = userData.ipAddress !== ipAddress;
        ...
        if (needsIpUpdate) {
          updates.push(
            db
              ...
              .set({
                ipAddress: ipAddress,
                userAgent: ctx.headers?.get("x-real-user-agent") || "",
                oauth: !hasPassword,
              })
              .where(eq(user.id, sessionUser.id))
          );
        } else if (userData.oauth !== !hasPassword) {
          updates.push(
            db
              .update(user)
              .set({
    ...
    stripe({
      ...
      subscription: {
        ...
        plans: [
          {
            ...
            priceId:
              ENVIRONMENT === "production"
                ? "price_1RrpUM12nSzGEbfJ2YnfVFtE"
                : "price_1RrhMe1d5ff1ueqRvBxqfePA",
          ...
          },
          {
            ...
            priceId:
              ENVIRONMENT === "production"
                ? "price_1RrpaS12nSzGEbfJhRq73THv"
                : "price_1RrhZW1d5ff1ueqRU3Ib2EXy",
          ...
          },
          {
            ...
            priceId:
              ENVIRONMENT === "production"
                ? "price_1Rrpbc12nSzGEbfJco6U50U7"
                : "price_1Rrha51d5ff1ueqRl8pBbUYM",
        ...
        onSubscriptionUpdate: async ({ subscription, event }) => {
          ...
          const newPlan =
            priceIdToPlanMap[newPriceId as keyof typeof priceIdToPlanMap];
